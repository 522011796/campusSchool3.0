  ...
  <div ref="commTeacherList">
    ...
    <el-table ref="commTableRef" :data="tableTeacherCommData"
      ...
      <el-table-column align="center" v-if="setType == 'radio'">
        <template slot-scope="scope">
          <my-radio :sel-value="commSelUserVal" v-bind="_selValue" :label="scope.row.teacher_id" @change="_handleChange(scope.row)"><span></span></my-radio>
        </template>
      </el-table-column>
    ...
    methods: {
      _initTeacher(){
        ...
        for (let i = 0; i < this.tableData.length; i++){
          let sel = inArray(this.tableData[i], this.commSelUserArr, 'teacher_id');
          if (sel > -1){
            this.commFlag = true;
      ...
      _handleSelectionSelect(event, row){
        if (event){//选中
          ...
          let checked = inArray(row, this.commSelUserArr, 'teacher_id');
      ...
      _handleSelectionAllSelect(selection){
        ...
        for (let i = 0; i < this.tableTeacherCommData.length; i++){
          if (selection == true){
            ...
            let checked = inArray(this.tableTeacherCommData[i], this.commSelUserArr, 'teacher_id');
            if (checked == -1){
      ...
      _handleSelect(){
        ...
        for (let i = 0; i < arr.length; i++){
          let sel = inArray(arr[i], arrTempUser, 'teacher_id');
          if (sel > -1){
            selArr.push(arr[i]);
