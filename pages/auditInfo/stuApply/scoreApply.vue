<template>
  <div class="container">
    <layout-tb>
      <template slot="tag">学分审批</template>
      ...
      <div slot="content">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-popover trigger="hover" placement="top" popper-class="custom-table-popover">
                <div class="text-center">{{scope.row.double1}}</div>
                <div slot="reference" class="name-wrapper moon-content-text-ellipsis-class">
                  {{scope.row.double1}}
                </div>
              </el-popover>
    ...
    <!--审批详细-->
    <drawer-layout-right @changeDrawer="closeDrawerDialog" :hide-footer="false" :visible="drawerVisible" size="550px" :title="$t('学分审批')" @right-close="cancelDrawDialog">
      <div slot="content">
...
<script>
  ...
  export default {
    ...
    components: {MyPagination,LayoutTb,MySelect,MyUserType,MyDatePicker,MyInputButton,DialogNormal,DrawerLayoutRight,AuditStatusButton},
    data(){
      return {
        tableData: [],
        searchDate: [],
