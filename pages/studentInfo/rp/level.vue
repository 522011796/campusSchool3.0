<template>
  <div class="container">
    <layout-tb>
      <template slot="tag">级别管理</template>

      <div slot="tab">
        <el-row>
          <el-col :span="6">
            <el-button size="small" type="primary"  icon="el-icon-plus" @click="addInfo($event)">{{$t("添加级别")}}</el-button>
          </el-col>
    ...
    <dialog-normal :visible="modalVisible" :title="$t('级别设置')" @close="closeDialog" @right-close="cancelDialog">
  ...
  export default {
    ...
    methods: {
      ...
      handleOkChange(data) {
        ...
        url = common.audit_re_static_del;
        params = this.$qs.stringify(params);
